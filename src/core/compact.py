...
async def _full_compact(messages: list[Message]) -> list[Message]:
    ...
    # Keep the last assistant message only if it is plain text. Assistant
    # messages containing tool calls must be followed immediately by matching
    # tool messages in OpenAI-compatible chat history; after full compaction
    # those tool messages are intentionally summarized away.
    for msg in reversed(messages):
        if msg.role == "assistant" and not msg.metadata.get("compacted"):
            has_tool_calls = not isinstance(msg.content, str) and any(
                getattr(block, "type", None) == "tool_use"
                for block in msg.content
            )
            if not has_tool_calls and msg.text_content != compacted[-1].text_content:
                compacted.append(msg)
            break
