...
def main():
    ...
    if not (os.environ.get("JINA_API_KEY") or os.environ.get("JINA_API_KEYS")):
        logger.warning(
            "JINA_API_KEY/JINA_API_KEYS not set — web_fetch will use direct fetch + local extraction. "
            "Set JINA_API_KEY or comma-separated JINA_API_KEYS for better content extraction "
            "(JS-rendered pages, PDFs)."
        )
